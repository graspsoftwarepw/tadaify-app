...
- 2026-05-03 · fix: BR-OTP-RATE-LIMIT-001 — OTP resend rate-limit: 3/session UI cap + "You can resend in {N}s" copy fix + 3/24h backend guard per email-handle pair; new table `otp_rate_limit_attempts` (service_role only, email_hash PII-safe); shared `OtpResendControl` component (register+login); 4 unit test bundles (U1-U4, 19 tests) + Playwright spec S1-S5 (otp-rate-limit.spec.ts) — Issue tadaify-app#179, DEC-342=A
