...
Lazy-loaded detailed guidance for Claude agents working on
`graspsoftwarepw/tadaify-app`.

Root `CLAUDE.md` is the always-active router. This file preserves detailed
repository context that agents must load when the task touches the matching
area.
...
- **Branch–issue link (mandatory):** every branch must be linked to its GitHub issue through the approved Agents Orchestrator token/App path before code is written. Do not use `gh`; `feature-checklist §0.7` HARD-blocks PRs without the link.
