...
`app-block-editor.html` overhauls the `Social icons row` block type per user feedback during PR #54 review. The fixed 6-platform input list is replaced with an addable, removable, draggable card list and a per-platform icon-style picker. Plus a small global change: an "A/B test" identity chip in the Content section header so the feature reads as a named capability across all 13 block types, not a footnote.
...
- **Block type coverage.** Picker now appears on: Link button (default `spotify`), Newsletter signup (`ctaIcon` default `send`), Product (`ctaIcon` default `shopping-cart`; renamed from "Shop product" 2026-04-26 per FIX-SHOP-001 — block now links to creator's external store, no native shop in MVP), Heading / text (optional leading `icon`), Countdown timer (label `icon` default `flame`), Live stream (`ctaIcon` default `play`), FAQ / accordion (optional `sectionIcon`). Image / Video / Embed / Divider / Custom HTML / Social icons row keep their existing visual treatment (Social row uses its own platform-handle picker — different concept, not conflated).
