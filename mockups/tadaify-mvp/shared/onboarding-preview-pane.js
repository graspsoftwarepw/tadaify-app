...
(function () {
  ...
  function renderPreviewSrcdoc(state, viewport) {
    ...
    return (
      ...
      'font-family:Inter,system-ui,sans-serif;width:' + width + 'px;}' +
  ...
  var PANE_CSS = (
    '.tdf-pp{position:sticky;top:24px;display:flex;flex-direction:column;gap:12px;' +
      'background:var(--bg-elevated,#fff);border:1px solid var(--border,#E5E7EB);' +
      'border-radius:18px;padding:14px;box-shadow:0 12px 30px -18px rgba(15,23,42,0.18);}' +
    '.tdf-pp-head{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;}' +
    '.tdf-pp-tabs{display:inline-flex;background:var(--bg-sunken,#F3F4F6);border-radius:99px;padding:3px;}' +
    '.tdf-pp-tabs button{appearance:none;background:transparent;border:0;padding:6px 12px;font:inherit;' +
      'font-size:12px;font-weight:600;color:var(--fg-muted,#4B5563);cursor:pointer;border-radius:99px;}' +
    '.tdf-pp-tabs button.active{background:#fff;color:var(--brand-primary,#6366F1);box-shadow:0 1px 4px rgba(15,23,42,0.08);}' +
    '.tdf-pp-theme{appearance:none;background:transparent;border:1px solid var(--border,#E5E7EB);border-radius:99px;' +
      'padding:6px 12px;font-size:12px;font-weight:600;color:var(--fg-muted,#4B5563);cursor:pointer;}' +
    '.tdf-pp-stage{background:repeating-linear-gradient(45deg,#F9FAFB,#F9FAFB 8px,#F3F4F6 8px,#F3F4F6 16px);' +
      'border-radius:14px;padding:14px;display:flex;justify-content:center;align-items:flex-start;min-height:480px;}' +
    ...
    '.tdf-pp-meta{font-size:11px;color:var(--fg-subtle,#6B7280);text-align:center;letter-spacing:.05em;text-transform:uppercase;}' +
  ...
  function buildPane(host, opts) {
    ...
    host.innerHTML = (
      '<div class="tdf-pp-head">' +
        '<div class="tdf-pp-tabs" role="tablist" aria-label="Preview viewport">' +
          '<button type="button" data-vp="desktop">Desktop</button>' +
          '<button type="button" data-vp="tablet">Tablet</button>' +
          '<button type="button" data-vp="mobile" class="active">Mobile</button>' +
        '</div>' +
      ...
      '<div class="tdf-pp-stage">' +
        ...
        '</div>' +
      '</div>' +
      '<p class="tdf-pp-meta" data-vp-label>Mobile · 390 × 844</p>'
    );
    ...
    host.querySelectorAll('[data-vp]').forEach(function (btn) {
      ...
      btn.addEventListener('click', function () {
        host.querySelectorAll('.tdf-pp-tabs button').forEach(function (b) { b.classList.remove('active'); });
